    ...
    <div style={{ direction: 'rtl', maxWidth: 860, margin: '0 auto', padding: '1.5rem 1rem 3rem' }}>
      ...
      {loading ? (
        ...
        shown.map((t) => {
          ...
          const refFiles = (Array.isArray(t.referenceFiles) ? t.referenceFiles : []).map(parseFile).filter((f) => f.url);
          const delivFiles = (Array.isArray(t.deliverableFiles) ? t.deliverableFiles : []).map(parseFile).filter((f) => f.url);
          ...
          return (
            <div key={t.id} style={card}>
              ...
              {(() => {
                ...
                const renderFiles = (arr: { name: string; url: string }[]) => {
                  const media = arr.filter((f) => isImg(f.url) || isVid(f.url));
                  const docs = arr.filter((f) => !isImg(f.url) && !isVid(f.url));
                  return (
                    <>
                      {media.length > 0 && (
                  ...
                  <>
                    {refFiles.length > 0 && (
                      <div style={{ marginTop: 12 }}>
                        <div style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--foreground-muted,#6b7280)', marginBottom: 6 }}>📎 קבצי עזר לעבודה:</div>
                        {renderFiles(refFiles)}
                      </div>
                    )}
                    {delivFiles.length > 0 && (
                      <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid rgba(0,0,0,0.06)' }}>
                        <div style={{ fontSize: 12.5, fontWeight: 800, color: '#16a34a', marginBottom: 6 }}>✅ קובץ מאושר:</div>
                        {renderFiles(delivFiles)}
                      </div>
                    )}
                  </>
                );
