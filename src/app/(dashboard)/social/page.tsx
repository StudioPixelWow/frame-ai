  ...
  return (
    <div className="p-6 max-w-7xl mx-auto" dir="rtl">
      ...
      <PageHeader
        ...
        subtitle="תזמון פוסטים, ניתוח ביצועים, חיבור Postiz"
      ...
      <div className="grid grid-cols-5 gap-3 mb-8">
        {platformStats.map(p => (
          <button
            ...
            className={`bg-surface rounded-xl p-4 border transition-colors text-center ${
              platformFilter === p.id ? 'border-blue-500/50 bg-blue-500/10' : 'border-border hover:border-border-muted'
            }`}
          >
            <div className="text-2xl mb-1">{p.icon}</div>
            <div className="text-sm font-medium text-foreground">{p.label}</div>
            <div className="text-xs text-foreground-muted mt-1">{p.count} פוסטים · {p.published} פורסמו</div>
          </button>
        ))}
      </div>
      ...
      <div className="bg-gradient-to-l from-blue-500/10 to-cyan-500/10 rounded-xl p-4 border border-blue-500/20 mb-8">
        <h3 className="text-sm font-medium text-blue-700 mb-2">🇮🇱 זמני פרסום מומלצים לקהל ישראלי</h3>
        <div className="grid grid-cols-2 gap-2">
          {ISRAELI_BEST_TIMES.map((tip, i) => (
            <div key={i} className="text-xs text-foreground-muted">
              <span className="text-foreground font-medium">{tip.platform}:</span> {tip.times}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-bold text-foreground">ציר זמן פוסטים</h2>
        <div className="text-sm text-foreground-subtle">{posts.length} פוסטים</div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-foreground-muted">אין פוסטים מתוזמנים</p>
            ...
            <div key={date}>
              ...
              <div className="space-y-2">
                {postsByDate[date].map((post: any) => {
                  ...
                  return (
                    <div key={post.id} className="bg-surface rounded-xl p-4 border border-border hover:border-border-muted transition-colors">
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-3 flex-1">
                          <span className="text-xl">{pInfo?.icon || '📱'}</span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-sm font-medium text-foreground">{post.clientName || 'לקוח'}</span>
                              <span className="text-xs text-foreground-subtle">{pInfo?.label}</span>
                        ...
                        <div className="text-left text-xs text-foreground-subtle">
                      ...
                      {post.status === 'published' && (post.likes > 0 || post.comments > 0) && (
                        <div className="flex gap-4 mt-3 pt-3 border-t border-border text-xs text-foreground-subtle">
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1f2e] rounded-2xl p-6 w-full max-w-lg border border-border">
            <h2 className="text-xl font-bold text-foreground mb-6">פוסט חדש</h2>
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm text-foreground-muted block mb-1">פלטפורמה</label>
                  ...
                  {PLATFORMS.map(p => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-center text-sm transition-colors ${
                        form.platform === p.id ? 'bg-blue-600 text-white' : 'bg-surface text-foreground-muted'
                      }`}
                    >
                      {p.icon}
                    </button>
              ...
              <div>
                <label className="text-sm text-foreground-muted block mb-1">תוכן</label>
                <textarea
                  value={form.content}
                ...
                />
                <div className="text-xs text-foreground-subtle mt-1">{form.content.length} תווים</div>
              </div>
              <div>
                <label className="text-sm text-foreground-muted block mb-1">האשטגים (מופרד בפסיקים)</label>
                <input
                  value={form.hashtags}
