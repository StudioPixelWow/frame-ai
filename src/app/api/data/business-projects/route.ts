...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[API] POST /api/data/business-projects ✅ persisted id=${id} service=${svc} templates=${seed.templatesFound} milestones=${seed.inserted}${seed.error ? ` error="${seed.error}"` : ''} payments=${paymentResult.ok ? 'OK' : 'FAILED'}${paymentResult.error ? ` payErr="${paymentResult.error}"` : ''}`
    );
