...
function validateItemQuality(item: GeneratedItem, index: number): string[] {
  ...
  // Caption must be at least 300 chars (target 350-400: hook + body + CTA + hashtags + emojis)
  if (item.caption.length < 300) {
    issues.push(`Item ${index}: caption too short (${item.caption.length} chars, min 300, target 350-400) — must be a full social media post with hook, body, CTA, hashtags, and 2-3 emojis`);
  }
...
3. graphicText — זהו הטקסט שנכתב על הגרפיקה/התמונה עצמה. חייב להיות קצר, חד, פאנצ׳י — כמו HEADLINE של קמפיין פרסומי. 1-2 שורות בלבד. תופס עין מיד, מעורר סקרנות, גורם לעצור בסקרול. שורה 1: hook חד. שורה 2 (אופציונלית): פאנץ׳/הבטחה. מינימום 8 תווים/שורה.
   ✅ "הכסף שלך עובד בשבילך?\\nגלה איך תוך 30 יום" ✅ "3 טעויות שעולות לך אלפים" ✅ "מה שהמתחרים מסתירים" ❌ "קרא עוד", "גלה עוד", "בואו", "חג שמח"

4. hook — משפט פתיחה שמוכר את אותו רעיון מרכזי. ספציפי, רגשי או פרובוקטיבי. מינימום 15 תווים. חייב להתאים ל-title.

5. caption — זהו הפוסט המלא שמתפרסם ברשתות חברתיות ומלווה את הגרפיקה. חייב להיות ארוך, מעניין, יצירתי ומושך. אורך מחייב: 600-800 תווים (ארוך משמעותית!). מבנה מחייב לפי סטנדרט ישראלי עם חלוקה לפסקאות:
   פסקה 1 — Hook: משפט פתיחה חד שעומד לבד לפני חיתוך "עוד". שאלה/הצהרה/מספר/ניגוד. (שורה ריקה אחרי)
   פסקה 2 — הקשר: 2-3 משפטים שמרחיבים את ה-hook, נותנים רקע או מציגים בעיה. (שורה ריקה אחרי)
   פסקה 3 — ערך/תובנה: 2-3 משפטים עם סיפור, נתון, טיפ מעשי או תובנה מקצועית שגורמת לקהל לעצור ולקרוא. (שורה ריקה אחרי)
   פסקה 4 — CTA: קריאה לפעולה ברורה עם ← (קישור בביו / שלחו "מעוניין" בוואטסאפ / תייגו חבר). (שורה ריקה אחרי)
   פסקה 5: 5-10 האשטגים — מיקס עברית+אנגלית, כולל #שיווק_דיגיטלי #עסקים_בישראל + האשטגים ניש ספציפיים.
   אימוג׳ים: חובה בדיוק 3 אימוג׳ים רלוונטיים פזורים בגוף הטקסט — אחד ב-hook, אחד בגוף, אחד ב-CTA. לא יותר ולא פחות מ-3!
   חובה: כל פסקה מופרדת בשורה ריקה (\\n\\n). הטקסט לא ברצף אחד ארוך — חלוקה ברורה לפסקאות קצרות ונעימות לקריאה.
   כל הגוף בעברית, זה צריך להיות פוסט שלם, עשיר ומעמיק שמושך engagement ושיתופים.
...
- graphicText: טקסט פרסומי חד שנכתב על הגרפיקה עצמה — 1-2 שורות, כמו headline מודעה. תופס עין, מוכר, לא גנרי!
- hook: משפט פתיחה שמוכר את אותו רעיון (15+ תווים)
- caption: פוסט שלם, ארוך ויצירתי לרשתות (600-800 תווים!) — מבנה מחייב עם חלוקה לפסקאות: פסקה 1 = Hook חד + אימוג׳י (שאלה/הצהרה/מספר). פסקה 2 = הקשר 2-3 משפטים. פסקה 3 = ערך/סיפור/תובנה 2-3 משפטים + אימוג׳י. פסקה 4 = CTA עם ← + אימוג׳י. פסקה 5 = 5-10 האשטגים (#עברית + #english). חובה בדיוק 3 אימוג׳ים! כל פסקה מופרדת ב-\\n\\n — לא הכל ברצף!
...
[
  {
    ...
    "graphicText": "טקסט פרסומי חד וקצר שנכתב על הגרפיקה עצמה — 1-2 שורות מקסימום, כמו headline מודעה. חד, פאנצ׳י, תופס עין. לא גנרי!\\nשורה 2 אופציונלית: הבטחה/פאנץ׳",
    "caption": "פוסט שלם, ארוך ויצירתי (600-800 תווים!). מבנה מחייב עם חלוקה לפסקאות:\n\nפסקה 1: Hook חד + אימוג׳י — שאלה/הצהרה/מספר שעומד לבד\n\nפסקה 2: הקשר — 2-3 משפטים שמרחיבים את ה-hook ונותנים רקע\n\nפסקה 3: ערך/תובנה — 2-3 משפטים עם סיפור או טיפ מעשי + אימוג׳י\n\nפסקה 4: ← CTA ספציפי + אימוג׳י (קישור בביו / שלחו מעוניין בוואטסאפ)\n\n#האשטג_עברית #hashtag_english (5-10 האשטגים מיקס)\n\nחובה בדיוק 3 אימוג׳ים! כל פסקה מופרדת ב-\\n\\n",
...
function generateCaptionDeterministic(title: string, platform: string, index: number): string {
  const templates = [
    `${title} 🎯\n\nאני אגיד לכם משהו שרוב האנשים לא רוצים לשמוע — אבל צריכים.\nבעולם שבו כולם עושים את אותו הדבר, ההבדל היחיד הוא איכות. ואיכות מתחילה בבחירה — בבחירה של מי עומד מאחורי העסק שלכם, מי מוביל את השיווק, ומי באמת מבין את הקהל שלכם.\n\nאנחנו לא מאמינים בקיצורי דרך, אבל כן מאמינים בתוצאות שמדברות בעד עצמן. הלקוחות שלנו רואים שינוי אמיתי — לא הבטחות, לא סלוגנים ריקים, אלא מספרים שעולים, פניות שנכנסות, ועסק שצומח 💪\n\nאז מה הבחירה שלכם היום? הגיע הזמן להפסיק לדחות ולהתחיל לפעול.\n← שלחו לנו הודעה ונדבר על זה 📩\n\n#עסקים #שיווק #צמיחה #שיווק_דיגיטלי #עסקים_בישראל #תוצאות #הצלחה`,
    `${title} 🚀\n\nלפני שנה חשבנו שהגענו לתקרה. היום? אנחנו במקום אחר לגמרי — ולא בגלל מזל.\nבגלל אסטרטגיה, עקביות, ותוצאות שמדברות בעד עצמן. כל יום התחלנו עם שאלה אחת: מה אפשר לשפר?\n\nמה שעשה את ההבדל? ההחלטה להפסיק לנחש ולהתחיל לעבוד עם נתונים אמיתיים. כל קמפיין, כל פוסט, כל שקל — נמדד ומותאם. ברגע שמתחילים לעבוד ככה, הכל משתנה 💡\n\nרוצים לדעת איך עושים את זה? הסוד לא בתקציב — הוא בגישה.\n← הקישור בביו, דברו איתנו 🔗\n\n#תוצאות #צמיחהעסקית #הצלחה #שיווק_דיגיטלי #אסטרטגיה #עסקים_בישראל #נתונים`,
    `${title} 💡\n\nהנה העניין — אין קיצורי דרך. אבל יש דרך חכמה, ויש דרך שמבזבזת לכם זמן וכסף.\nאנחנו בחרנו בדרך החכמה ואנחנו פה כדי לעזור גם לכם לעשות את אותו הדבר. כי ברגע שמבינים מה עובד ומה לא — הכל מתחיל להסתדר.\n\nבמקום לנסות הכל ולקוות לטוב — בואו נבנה תוכנית שעובדת. עם מטרות ברורות, לוחות זמנים שאפשר לעמוד בהם, ותוצאות שאפשר למדוד. זה לא קסם — זו מקצועיות 🔥\n\nמעניין אתכם? הגיע הזמן לקחת את העסק שלכם לשלב הבא.\n← תכתבו לנו ״מעניין״ בתגובות 📩\n\n#טיפים #ערך #עסקיםקטנים #שיווק #צמיחהעסקית #יזמות #תוכנית_עבודה`,
    `${title} 👂\n\nהדבר הכי חשוב שלמדנו השנה? להקשיב ללקוחות. לא רק לשמוע — להקשיב באמת.\nכל שיפור, כל מוצר חדש, כל שירות — נולד מהמילים שלכם. כשלקוח אומר ״הייתי רוצה ש...״ — אנחנו לא רושמים ושוכחים. אנחנו בונים.\n\nוככה נולדו הדברים הכי טובים שעשינו השנה. כי אף אחד לא מכיר את הצרכים שלכם יותר טוב מכם. אנחנו פשוט יודעים לקחת את מה שאתם אומרים ולהפוך את זה למשהו שעובד 💪\n\nספרו לנו — מה הדבר הבא שהייתם רוצים? אנחנו מקשיבים.\n← כתבו בתגובות, נשמח לשמוע 👇\n\n#לקוחות #חדשנות #ביחד #שיווק_דיגיטלי #עסקים_בישראל #פידבק #שירות`,
  ];
  return seedPick(templates, index);
}
