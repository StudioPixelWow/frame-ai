...
function validateItemQuality(item: GeneratedItem, index: number): string[] {
  ...
  // Caption must be at least 80 chars (real social caption, not placeholder)
  if (item.caption.length < 80) {
    issues.push(`Item ${index}: caption too short (${item.caption.length} chars) — must be a full social media post with hook, body, CTA, hashtags`);
  }
...
5. caption — זהו הפוסט המלא שמתפרסם ברשתות חברתיות ומלווה את הגרפיקה. חייב להיות ארוך, מעניין, יצירתי ומושך. מינימום 150 תווים. מבנה מחייב לפי סטנדרט ישראלי:
...
- caption: פוסט שלם ויצירתי לרשתות (150+ תווים) — מבנה מחייב: שורה 1 = Hook חד (שאלה/הצהרה/מספר). שורות 2-4 = גוף עם ערך/סיפור/תובנה. שורה 5 = CTA עם ← (קישור בביו/וואטסאפ). שורה אחרונה = 5-10 האשטגים (#עברית + #english). שורות מופרדות
