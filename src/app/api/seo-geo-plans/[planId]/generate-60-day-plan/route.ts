...
async function generateAIKeywords(plan: any): Promise<string[]> {
  ...
  try {
    const result = await generateWithAI(
      `אתה מומחה SEO ופרסום ממומן ישראלי. החזר רשימת 20 ביטויי מפתח בעברית שאנשים מחפשים בגוגל, מותאמים לעסק.

כללי מורפולוגיה עברית לביטויים:
- כלול צורות סמיכות ("חברת שיווק" + "חברה לשיווק")
- כלול ביטויים מקומיים ("[שירות] ב[עיר]", "[שירות] באזור [אזור]")
- כלול ביטויי כוונה מסחרית ("מחיר", "עלות", "כמה עולה", "השוואה")
- כלול ביטויי זנב ארוך (4+ מילים, ספציפיים)
- סווג לפי intent: מסחרי, מידעי, ניווטי, עסקאי

החזר JSON בלבד: {"keywords": ["ביטוי 1", "ביטוי 2", ...]}`,
      `הנה המידע על העסק:\n${context}\n\nצור 20 ביטויי SEO רלוונטיים בעברית.\n\nחלוקה נדרשת:\n- 5 ביטויים ראשיים (Core) — השירות/מוצר המרכזי\n- 5 ביטויי שירותים — וריאציות של השירותים\n- 5 ביטויים מקומיים — "[שירות] ב[עיר]"\n- 5 ביטויי זנב ארוך (Long Tail) — שאלות ספציפיות שאנשים מחפשים\n\nJSON בלבד.`,
...
async function generateAIArticles(plan: any, keywords: string[]): Promise<any[]> {
  ...
  try {
    const result = await generateWithAI(
      `אתה מומחה תוכן, SEO ו-GEO ישראלי עם 15 שנות ניסיון. אתה יוצר תוכניות תוכן אסטרטגיות שמדרגות בגוגל וגם מקבלות אזכור במנועי AI (Google AI Overview, ChatGPT, Perplexity, Gemini).`,
      `העסק: ${businessName} (${businessType})
ביטויי מפתח: ${topKeywords}
השנה הנוכחית: ${new Date().getFullYear()}

צור 8 נושאי מאמרים אסטרטגיים — 4 מאמרי SEO קלאסיים + 4 מאמרי GEO (מותאמים לציטוט ב-AI):

מאמרי SEO (1-4): מאמרים ארוכים (1500+ מילים) שמקדמים ביטויי מפתח מסחריים.
מאמרי GEO (5-8): מאמרים במבנה שמנועי AI אוהבים לצטט:
  - כותרת בפורמט שאלה ("מה זה X?", "כמה עולה X?", "איך לבחור X?")
  - מבנה: הגדרה → שלבים ממוספרים → יתרונות/חסרונות → FAQ
  - תשובות ישירות בפסקה ראשונה (50-80 מילים — אופטימלי לציטוט AI)

חשוב:
- אל תשתמש בשנים ישנות בכותרות — ${new Date().getFullYear()} בלבד
- כלול בכותרות: מספרים, שנה, מילות כוח ("מדריך מלא", "כל מה שצריך לדעת")
...
{"articles": [
  {
    ...
    "wordCount": 1500,
    "contentType": "seo|geo",
    "whyThisArticle": "למה מאמר זה חשוב לקידום בגוגל ובמנועי AI"
  }
]}`,
...
async function generateFullArticle(plan: any, article: any): Promise<any> {
  ...
  try {
    const result = await generateWithAI(
      `אתה כותב תוכן SEO ו-GEO מקצועי בעברית עם 15 שנות ניסיון בשוק הישראלי. אתה כותב מאמרים מלאים, עמוקים ומקצועיים שמדרגים גבוה בגוגל וגם מצוטטים על-ידי מנועי AI (Google AI Overview, ChatGPT, Perplexity).

כללי SEO מורפולוגי עברי:
- שלב את ביטוי המפתח ב-5+ צורות שונות: בסיסי, סמיכות ("חברת X"), רבים ("שירותי X"), מילות יחס ("בX", "לX"), ה' הידיעה ("הX")
- כתוב כותרות H2 עם הביטוי בצורות שונות

כללי GEO (נראות במנועי AI):
- פסקה ראשונה = תשובה ישירה (50-80 מילים) — זו הפסקה שמנועי AI מצטטים
- כלול סקשן "מה זה [TOPIC]?" עם הגדרה אנציקלופדית
- השתמש ברשימות ממוספרות (מנועי AI מעדיפים שלבים ברורים)
- כלול FAQ עם תבניות ישראליות: "כמה עולה", "מה ההבדל", "איך לבחור"

E-E-A-T ישראלי:
- ציין שנות ניסיון, מספר לקוחות, הסמכות
- הפנה למקורות ישראליים (TheMarker, Calcalist, Globes, מחקרים אקדמיים)
- שפה סמכותית ומקצועית

כללים כלליים:
- כתוב בעברית מקצועית וטבעית
- אורך מינימלי: 1500 מילים
- כתוב כותרות H2 ו-H3 ברורות
- כלול דוגמאות מעשיות ונתונים ישראליים
- סיים עם FAQ (3 שאלות ותשובות בתבניות ישראליות)
- סיים עם CTA לעסק
