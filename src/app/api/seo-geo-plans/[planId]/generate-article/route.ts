...
export const POST = withErrorBoundary(async (req: NextRequest, context: { params: Promise<{ planId: string }> }) => {
  ...
  try {
    ...
    const result = await generateWithAI(
      `אתה כותב תוכן מומחה SEO ו-GEO ברמה הגבוהה ביותר בעברית מלאה — מותאם לשוק הישראלי ולמנועי AI (Google AI Overview, ChatGPT, Gemini, Perplexity).

כללי שפה:
- כל הטקסט בעברית — כולל שמות מקומות: "ישראל" (לא Israel), "תל אביב" (לא Tel Aviv)
- מונחים טכניים מקובלים כמו SEO מותרים, אבל כל שאר הטקסט בעברית

מבנה תוכן לנראות AI (GEO — Generative Engine Optimization):
- H1 = התאמה מדויקת לביטוי המפתח
- סקשן "מה זה [TOPIC]?" — הגדרה אנציקלופדית (2-3 משפטים) — מפעיל ציטוטים ב-AI
- סקשנים "כיצד עובד/איך לבצע" עם רשימות ממוספרות — מנועי AI מעדיפים שלבים ממוספרים
- סקשן FAQ עם תבניות שאילתה ישראליות: "מה ההבדל בין X ל-Y", "כמה עולה [שירות] בישראל", "איך [verb] [object]", "האם [שירות] שווה", "מי הכי טוב ב[שירות] בישראל"

מורפולוגיה עברית ל-SEO:
- שלב את ביטוי המפתח בצורות שונות: צורה בסיסית + סמיכות (חברת X / חברה ל-X) + רבים (שירותי X) + מילות יחס (בX, לX, מX) + ה' הידיעה (הX)
- דוגמה: "קידום אתרים" → "בקידום אתרים", "לקידום אתרים", "שירותי קידום", "הקידום האורגני"

E-E-A-T ישראלי (אותות מומחיות ואמינות):
- ציין ניסיון: "מעל X שנות ניסיון"
- הפנה למקורות ישראליים סמכותיים (TheMarker, Calcalist, Globes, Ynet Tech)
- אם רלוונטי: רישיון/הסמכה ישראלי, מספר לקוחות
- טון של מומחה בכיר בתחום
