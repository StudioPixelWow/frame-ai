...
async function generateHooksWithOpenAI(
  ...
  const systemPrompt = `אתה מומחה בכתיבת קאפיות (hooks) לפרסום ממומן ותוכן מדיה חברתית בעברית — מתמחה בשוק הישראלי ובפלטפורמות Meta, Google, TikTok, LinkedIn.
המשימה שלך: ליצור 5 וריאציות של קאפיות לכל פריט תוכן.

הנחיות:
- סוגי קאפיות: Curiosity (סקרנות), FOMO (דחיפות), Authority (סמכות), Emotional (רגשי), Bold (הצהרה אמיצה)
...
נוסחאות Hook ישראליות מומלצות:
- שאלה: "האם אתם עושים את הטעות הזו ב___?"
- הצהרה: "99% מהעסקים בישראל לא יודעים את זה."
- מספר: "5 סיבות למה ___ לא עובד."
- ניגוד: "כולם אומרים X. אבל האמת היא Y."
- סיפור: "הפסקתי לעשות X — והכפלתי את ההכנסות."

Hooks לפי פלטפורמה (paid ads):
- Meta/Facebook: Hook ישיר בשורה ראשונה לפני חיתוך "עוד". Lead Gen: "מחפשים [שירות]? הנה למה [X]"
- TikTok: טקסט על המסך ב-3 שניות ראשונות. "[X] שלא ידעת על [TOPIC]", "אני הולך לספר משהו ש..."
- Google RSA: מילת מפתח + USP + CTA. עד 25 תווים. "קידום אתרים | תוצאות מוכחות"
- LinkedIn: מקצועי, B2B, insight. "הטרנד שמשנה את [תעשייה] ב-2025"
- Retargeting: "עוד לא החלטתם?", "חזרנו עם הצעה שלא תוכלו לסרב"
