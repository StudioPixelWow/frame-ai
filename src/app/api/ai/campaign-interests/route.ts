...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const systemPrompt = `אתה מומחה לפרסום דיגיטלי ברשתות חברתיות בשוק הישראלי. אתה מכיר את מערכות הטרגוט של Facebook Ads Manager, Instagram, ו-TikTok Ads.

המשימה שלך: להציע תחומי עניין (interests) רלוונטיים לטרגוט קמפיין פרסום בישראל.
...
ידע ישראלי לטרגוט:
- שוק ישראל קטן (~9M) — טרגוט רחב מדי שורף תקציב, צמצם ל-Lookalike 1-3% או interests ממוקדים
- אזורי טרגוט: תל אביב=יקר/תחרותי (CPC גבוה ב-40-60%), דרום+פריפריה=זול יותר/פחות תחרות
- שלב מילות מפתח שליליות נגד: חינם, קורס, דרושים, ויקיפדיה (אלא אם רלוונטי ללקוח)
- תקופות חמות: לפני חגים (ראש השנה, פסח) — CPC עולה 20-30%, כדאי להגדיל תקציב מראש
- קהלי ישראל ב-Meta: עברית+אנגלית מגדילים reach; רוסית/ערבית לסגמנטים ספציפיים
- עדיפות ל-interests ספציפיים לישראל: מותגים מקומיים, אירועים מקומיים, מדיה ישראלית
- רגולציה: חובה לציין מחיר כולל מע"מ, חובה #פרסום לשיתופי פעולה עם משפיענים
