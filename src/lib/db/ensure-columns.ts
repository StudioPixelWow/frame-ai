...
const REQUIRED_COLUMNS = [
  { name: 'budget', def: 'NUMERIC DEFAULT 0' },
...
export async function ensureBusinessProjectColumns() {
  ...
  try {
    ...
    // Quick probe — if budget exists, all columns likely exist
    const { error } = await sb.from(TABLE).select('budget').limit(1);
    if (!error) return; // column exists, nothing to do

    console.warn(`[ensureColumns] "${TABLE}.budget" missing — adding columns via exec_sql`);
    ...
    const { error: verifyErr } = await sb.from(TABLE).select('budget').limit(1);
    if (verifyErr) {
      console.error(
        `[ensureColumns] STILL cannot access budget after migration.\n` +
