...
async function generateFAQsWithAI(
  ...
  const systemPrompt = `אתה מומחה SEO שיוצר שאלות נפוצות (FAQ) בעברית עבור אתרי אינטרנט.
...
צור 4-6 שאלות נפוצות (FAQ) בעברית:
- שאלות שאנשים באמת שואלים בגוגל
- תשובות קצרות ומדויקות (2-4 משפטים)
- כלול מילות מפתח בצורה טבעית
- שאלות מגוונות: מה, למה, איך, כמה, מתי
