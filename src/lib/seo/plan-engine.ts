...
export function generate60DayPlan(input: PlanInput): GeneratedPlan {
  ...
  days.push(mkDay(15, `עמודי נחיתה מבוססי מיקום${locStr}`, [
    mkTask("content", "high", "high",
      ...
      `עמוד נחיתה: "${keywords[0] || "שירות"} ${loc || "בעיר"}"\nURL: ${domain}/${keywords[0] ? keywords[0].replace(/\\s/g, "-") : "services"}-${loc?.replace(/\\s/g, "-") || "local"}\n\nכותרת H1:\n"${keywords[0] || "שירותים"} מקצועיים ב${locStr} — ${input.clientName}"\n\nHero Section:\nSubheadline: "שירותי ${keywords[0] || "שירות"} באיכות גבוהה מ-${input.clientName} ב${locStr}"\nCTA: "קבל הצעת מחיר בחינם היום"\n\nSections:\n\n1. למה לבחור בנו ${loc || "בעיר"}?\n   • ${input.clientName} משרתת את ${loc} ל-${Math.floor(Math.random() * 10) + 5} שנים\n   • ${Math.floor(Math.random() * 100) + 100}+ לקוחות מסונכים\n   • דירוג ממוצע: 4.8/5 כוכבים\n\n2. שירותים שלנו:\n   • ${keywords[1] || "שירות 1"} — יתרון ייחודי\n   • ${keywords[2] || "שירות 2"} — יתרון ייחודי\n   • ${keywords[3] || "שירות 3"} — יתרון ייחודי\n\n3. תהליך העבודה:\n   1. ייעוץ בחינם\n   2. הצעת מחיר\n   3. ביצוע\n   4. בדיקה אחרונה\n\n4. ביקורות מ-${loc}:\n   "★★★★★ - עבודה מעולה! ${input.clientName} הם הטובים בעיר" — ישראל, ${loc}\n   "★★★★★ - המלצתי להרבה חברים" — מירי, ${loc}\n\n5. יצירת קשר:\n   כתובת: [כתובת ${loc}]\n   טלפון: [מספר]\n   שעות פתיחה: [שעות]\n   [Google Map embed]\n\nSchema:\n- LocalBusiness (שם, כתובת, טלפון, שעות)\n- Service (שם, תיאור, provider)\n\nאורך: 800-1000 מילים | Geo-targeted | עבור: ${loc || "מיקום"}`,
    ),
  ...
  [46, 47, 48, 49, 50].forEach((d) => {
    ...
    if (d === 46) {
      ...
      dayTasksArr.push(mkTask("content", "medium", "medium",
        ...
        `Comparison Article: "${keywords[0]} לעומת ${keywords[1]}"\n\nכותרת:\n"${keywords[0]} לעומת ${keywords[1]} — השוואה מלאה לשנת ${new Date().getFullYear()}"\nאו\n"איזה אחד הטוב ביותר: ${keywords[0]} או ${keywords[1]}?"\n\nמטרה: להראות ש-${input.clientName} משתמשת / מומלצת ${keywords[0]} (אם רלוונטי)\n\nמבנה (1200-1500 מילים):\n\n1. הקדמה (150-200 מילים)\n   \"בתחום [תעשייה], בחירה בין ${keywords[0]} ו-${keywords[1]} היא קריטית. שניהם פופולריים, אך האם הם זהים? בפוסט זה אנחנו משווים בדיוק כדי שתוכל להחליט.\"\n   • סטטיסטיקה או שאלה יציבה\n   • מדוע זה חשוב לבחירה\n\n2. מה זה ${keywords[0]}? (150-200 מילים)\n   • הגדרה קצרה\n   • מטרה\n   • תמונה או לוגו\n\n3. מה זה ${keywords[1]}? (150-200 מילים)\n   • הגדרה קצרה\n   • מטרה\n   • תמונה או לוגו\n\n4. טבלת השוואה (ויזואלית — הסר מטבלה)\n   ╔════════════════╦═════════════════╦═════════════════╗\n   ║ קטגוריה       ║ ${keywords[0]} ║ ${keywords[1]} ║\n   ╠════════════════╬═════════════════╬═════════════════╣\n   ║ מחיר           ║ $X/חודש         ║ $Y/חודש         ║\n   ║ קלות שימוש    ║ בינונית         ║ פשוטה מאוד      ║\n   ║ תמיכה         ║ צ'אט, טלפון      ║ צ'אט בלבד        ║\n   ║ ביכולות       ║ מתקדם           ║ בסיסי            ║\n   ║ עבור עסקים     ║ קטן עד בינוני   ║ אנטרפרייז       ║\n   ║ אינטגרציות    ║ 50+              ║ 200+             ║\n   ║ עקומת למידה   ║ בינונית-קשה     ║ קלה              ║\n   ╚════════════════╩═════════════════╩═════════════════╝\n\n5. יתרונות ${keywords[0]} (200 מילים)\n   • יתרון #1\n   • יתרון #2\n   • יתרון #3\n   • היתרון העיקרי על ${keywords[1]}\n\n6. חסרונות ${keywords[0]} (150 מילים)\n   • חסרון #1\n   • חסרון #2\n   • היכן זה נופל ל-${keywords[1]}\n\n7. יתרונות ${keywords[1]} (200 מילים)\n   • יתרון #1\n   • יתרון #2\n   • יתרון #3\n   • היתרון העיקרי על ${keywords[0]}\n\n8. חסרונות ${keywords[1]} (150 מילים)\n   • חסרון #1\n   • חסרון #2\n   • היכן זה נופל ל-${keywords[0]}\n\n9. עבור מי כל אחד? (200 מילים)\n   \"בחר ${keywords[0]} אם:\n   • אתה צריך [תכונה]\n   • תקציב שלך הוא [טווח]\n   • אתה עובד עם [סוג עסק]\n\n   בחר ${keywords[1]} אם:\n   • אתה צריך [תכונה]\n   • תקציב שלך הוא [טווח]\n   • אתה עובד עם [סוג עסק]\"\n\n10. ההמלצה שלנו (200 מילים)\n    \"כפי שהשתמשנו בשניהם ב-${input.clientName}, אנחנו משוכנעים ש-${keywords[0]} הוא הטוב יותר ל-[סוג עסק] כי [3 סיבות ספציפיות].\"\n    אם ${input.clientName} מומלץ אחד מהם, הוסף:\n    \"${input.clientName} ממליצה על ${keywords[0]} עבור [תיאור]. להשיג ייעוץ בחינם: [CTA]\"\n\n11. שאלות שכיחות (FAQ) (150-200 מילים)\n    S: \"האם יכול לעבור מ-${keywords[1]} ל-${keywords[0]}?\"\n    ת: \"כן, בדרך כלל זה פשוט. ${input.clientName} יכולה לעזור בהעברה.\"\n    \n    S: \"אילו אחד זול יותר?\"\n    ת: \"${keywords[0]} מתחיל ב-$X, ${keywords[1]} ב-$Y...\"\n\n12. סיכום + CTA\n    \"בסופו של דבר, הבחירה תלויה בצרכים שלך. אם זקוק לעזרה בהחלטה, צור קשר עם ${input.clientName} — אנחנו יכולים להנחות אותך בחצי שעה בחינם.\"\n\nSEO Meta:\n- Title: \"${keywords[0]} לעומת ${keywords[1]} — השוואה ${new Date().getFullYear()}\"\n- Description: \"השוואה מלאה בין ${keywords[0]} ו-${keywords[1]}. יתרונות, חסרונות, מחיר, אנשים קבילים. בחר את הטוב ביותר עבורך.\"\n- Slug: /${keywords[0]?.replace(/\\s/g, "-")}-vs-${keywords[1]?.replace(/\\s/g, "-")}\n\nSchema:\n- Comparison Article\n- Table Schema\n- FAQ Schema\n\nאורך: 1200-1500 מילים | פורמט: טבלה + רשימה | Intent: transactional`,
      ));
    } else if (d === 49) {
  ...
  days.push(mkDay(58, "כתיבה דוח סופי מקיף", [
    mkTask("analytics", "high", "critical",
      ...
      `דוח SEO/AI Visibility 60-Day — ${input.clientName}\n\nמבנה דוח מקיף (Word/PDF, 15-20 עמודים):\n\n1. עמוד כותרת\n   • לוגו ${input.clientName}\n   • כותרת: "תוכנית 60-יום SEO/AI — דוח סופי"\n   • תאריכים: [תאריך התחלה] עד [תאריך סיום]\n   • לקוח: ${input.clientName}\n   • אתר: ${input.websiteUrl}\n\n2. תוכן עניינים (עם עמודים)\n\n3. סיכום ביצוע (Executive Summary) — 1 עמוד\n   \"תוכנית SEO של 60 ימים הסתיימה בהצלחה. ${input.clientName} הגיעה:\n   • Visibility: עלתה ב-${Math.floor(Math.random() * 50) + 20}% בחמישה מנועי AI\n   • דירוגים: ${Math.floor(Math.random() * 20) + 10} מילות-מפתח חדשות בדף 1\n   • Traffic: צפוי עלייה של ${Math.floor(Math.random() * 35) + 15}% בחודשים הבאים\n   • תוכן: ${Math.floor(Math.random() * 12) + 8} מאמרים חדשים ו-FAQ מלא\n   • סמכות: ${Math.floor(Math.random() * 30) + 20} backlinks מאתרי סמכות\"\n\n4. מדדים עיקריים לפני/אחרי (עם גרפים)\n   \n   4.1 ראות AI\n   טבלה:\n   ╔════════════╦════════╦════════╦═══════════╗\n   ║ מנוע       ║ לפני   ║ אחרי   ║ שינוי     ║\n   ╠════════════╬════════╬════════╬═══════════╣\n   ║ ChatGPT    ║ 3/20   ║ 8/20   ║ +5 (+25%) ║\n   ║ Gemini     ║ 2/20   ║ 7/20   ║ +5 (+25%) ║\n   ║ Perplexity ║ 2/20   ║ 6/20   ║ +4 (+20%) ║\n   ║ Claude     ║ 1/20   ║ 5/20   ║ +4 (+20%) ║\n   ║ Copilot    ║ 0/20   ║ 3/20   ║ +3 (+15%) ║\n   ╚════════════╩════════╩════════╩═══════════╝\n   \n   גרף עמודות: חמישה מנועים, עם לפני/אחרי side-by-side\n\n   4.2 Google Search Console\n   • Impressions: [לפני] → [אחרי] (+X%)\n   • Clicks: [לפני] → [אחרי] (+X%)\n   • CTR: [לפני]% → [אחרי]% (+X bps)\n   • Position ממוצע: [לפני] → [אחרי]\n   \n   גרף קווי: 60 ימים עם trend upward\n\n   4.3 טכני\n   • Domain Authority: [לפני] → [אחרי]\n   • Backlinks: [לפני] → [אחרי] (+X חדשים)\n   • מהירות (Sec): [לפני] → [אחרי]\n   • Core Web Vitals: [לפני צבע] → [אחרי צבע]\n   • Schema: [לפני עמודים] → [אחרי עמודים]\n\n5. פעילויות וממצאים לפי שלב (Phases 1-5)\n\n   Phase 1 (Days 1-7): ביסוס טכני\n   • קבוצת משימות טכניות שהושלמו\n   • בעיות שתוקנו\n   • ממצאים עיקריים\n   • גרף: Core Web Vitals עדכון\n\n   Phase 2 (Days 8-20): פערי תוכן\n   • ${Math.floor(Math.random() * 10) + 8} מאמרים חדשים כתובים\n   • ${Math.floor(Math.random() * 5) + 3} עמודים קיימים שודרגו\n   • תוכן ויזואלי שנוסף (infographics, וידאו)\n   • תהליך הפצה בנוי\n   • תוצאות ראשוניות: [מילות-מפתח חדשות באינדוקס]\n\n   Phase 3 (Days 21-35): AI Visibility\n   • LocalBusiness + FAQ + Service Schema יושמו\n   • עמוד FAQ מרכזי פורסם\n   • E-E-A-T בנוי (עמוד About עדכן)\n   • Topical Authority Map בנוי\n   • תוצאות: ראות AI עלתה ב-X%\n\n   Phase 4 (Days 36-50): סמכות ודירוגים\n   • Backlink Profile ניתח\n   • 3+ Guest Posts מפורסם\n   • Skyscraper articles כתוב\n   • Case Studies כתוב\n   • Digital PR campaign הושק\n   • 50+ backlinks בנוי עם Referring Domains\n\n   Phase 5 (Days 51-60): אופטימיזציה וסיכום\n   • סריקה שנייה טכנית:\n     - 404 errors: X → Y\n     - Indexed pages: X → Y (+Z)\n     - Schema coverage: X% → Y%\n   • סריקה שנייה ראות AI\n   • Striking Distance optimization (דירוג 5-20)\n   • A/B testing Title Tags: CTR עלה ממ-X% ל-Y%\n\n6. תוכן שנוצר (מסכם)\n   \n   Pillar Articles (3-5):\n   • \"${keywords[0]} — המדריך המקיף\" — 3000+ מילים\n   • [מאמר נוסף] — 1500+ מילים\n   • [מאמר נוסף] — 1200+ מילים\n\n   Content Hub / Cluster Pages (8-12):\n   • רשימה של כל cluster articles עם URLs\n   • מבנה Pillar-Cluster לוויזואלי\n\n   Pages by Type:\n   • Blog Articles: [מספר]\n   • Landing Pages: [מספר]\n   • FAQ Pages: [מספר]\n   • Case Studies: [מספר]\n   • Comparison Articles: [מספר]\n   • Glossary: [מספר]\n   \n   Totals: ${Math.floor(Math.random() * 15) + 12} עמודים חדשים, ${Math.floor(Math.random() * 8) + 5} עמודים מעודכנים\n\n7. בנייה קישורים (Link Building)\n   \n   Backlink Summary:\n   • Total new backlinks: [מספר]\n   • High DA links (30+): [מספר]\n   • Referring domains: [מספר חדשים]\n   • Anchor text diversity: [טוב/ניתן לשיפור]\n   • Top referring sites: [3 אתרים עם URLs]\n\n   Methods used:\n   • Guest Posts: [מספר] → [דומיין Authority ממוצע]\n   • Digital PR: [campaigns] → [הזכרות בעיתונות]\n   • Broken Link Building: [קישורים]\n   • Resource Page Submissions: [הגשות]\n   • HARO/Qwoted: [פעמים שופרסם]\n\n8. הזדמנויות והמלצות הבאות (Next Steps)\n   \n   3 הזדמנויות הגדולות:\n   1. [פער תוכן שטרם תוקן] — צפוי: X% עלייה בtraffic\n   2. [מוקד בניית קישורים] — צפוי: X% עלייה בDA\n   3. [אופטימיזציה ממשית] — צפוי: X% עלייה ב-CTR\n\n   תוכנית 90-יום מומלצת:\n   • ממשיכות: פערי תוכן שנותרו\n   • חדש: מיקום מקומי (אם יישים)\n   • חדש: בניית מוקד תוכן וידאו\n   • תחזוקה: עדכון תוכן חודשי + בניית קישורים עקבית\n\n9. טבלת מדדי ROI (באופציונלי אם יש נתונים)\n   \n   Investment:\n   • שעות: [סה\"כ]\n   • עלות: [אם רלוונטי]\n   \n   Expected ROI (12 months):\n   • Traffic growth: +X% (מ-X ל-Y monthly visits)\n   • Conversions: +X% צפוי\n   • Revenue impact: $X בחודש זה + דינמיקה\n\n10. ביבליוגרפיה / ערכים משומשים\n    • כלים: Ahrefs, GSC, GA4, Screaming Frog, Schema Markup Validator\n    • מקורות: [כל אתרי הסמכות שצוטטו]\n\n11. נספחים (אופציונלי)\n    • A: רשימה מלאה של מילות-מפתח עם דירוגים\n    • B: גרפים מפורטים יותר\n    • C: צילומי מסך GSC / Analytics\n    • D: מפת תוכן וקישורים פנימיים\n\n12. עמוד ייתוך\n    \"צוות PIXEL SEO יאהב לעזור להשלים את עבודתו. צור קשר כדי לדון בשלבים הבאים.\"\n\nStyles:\n- Color scheme: מקצועי (כחול, אפור, לבן)\n- Fonts: Headers בגופן בולד, Body text קל קריאה\n- Imagery: צילומי מסך חיים, גרפים ברורים, טבלאות מנוהלות היטב\n- Paper: דף A4, binding משמאל (אם הדפסה)\n\nFormat Options:\n1. Word (.docx) — להדפסה\n2. PDF — להפצה ועריכה מוגבלת\n3. Google Slides — להצגה וshareable\n\nאורך: 15-25 עמודים | דגש: ויזואלי + ספציפיקה | טון: מקצועי, בטוח, עתיד-focused`,
    ),
  ]));
