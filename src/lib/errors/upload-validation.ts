/**
 * PixelFrameAI — Upload File Validation
 *
...
import { PixelFrameError } from "./pixelframe-error";
 ...
 *
 * Returns `null` if the file is acceptable, or a `PixelFrameError`
...
): PixelFrameError | null {
  // Type check
  if (!ALLOWED_VIDEO_TYPES.has(file.type)) {
    const displayType = file.type || "unknown";
    return new PixelFrameError(
